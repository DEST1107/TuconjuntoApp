CREATE DATABASE IF NOT EXISTS tuconjuntoapp
...
CREATE TABLE conjuntos (
    ...
    CONSTRAINT ck_conjuntos_email CHECK (email_contacto IS NULL OR email_contacto LIKE '%@%')
) ENGINE=InnoDB;
...
CREATE TABLE torres (
    ...
    CONSTRAINT ck_torres_pisos CHECK (cantidad_pisos > 0),
...
CREATE TABLE apartamentos (
    ...
    CONSTRAINT ck_apartamentos_piso CHECK (piso >= 0),
    CONSTRAINT ck_apartamentos_coeficiente CHECK (coeficiente >= 0),
...
CREATE TABLE usuarios_apartamentos (
    ...
    CONSTRAINT ck_usuarios_apartamentos_fechas CHECK (fecha_fin IS NULL OR fecha_fin >= fecha_inicio),
...
CREATE TABLE conceptos_pago (
    ...
    CONSTRAINT ck_conceptos_pago_valor CHECK (valor_base >= 0)
) ENGINE=InnoDB;
...
CREATE TABLE cuentas_cobro (
    ...
    CONSTRAINT ck_cuentas_valor_total CHECK (valor_total > 0),
    CONSTRAINT ck_cuentas_saldo CHECK (saldo_pendiente >= 0 AND saldo_pendiente <= valor_total),
    CONSTRAINT ck_cuentas_fechas CHECK (fecha_vencimiento >= fecha_emision),
...
CREATE TABLE incidencias (
    ...
    CONSTRAINT ck_incidencias_fechas CHECK (fecha_cierre IS NULL OR fecha_cierre >= fecha_reporte),
...
CREATE TABLE noticias (
    ...
    CONSTRAINT ck_noticias_fechas CHECK (fecha_expiracion IS NULL OR fecha_expiracion >= fecha_publicacion),
